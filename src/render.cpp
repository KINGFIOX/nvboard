...
SDL_Texture *new_texture(SDL_Renderer *renderer, int w, int h, int r, int g,
                         int b) {
...
void draw_surrounding_line(
    SDL_Renderer *renderer, SDL_Rect r,
...
static void draw_str_internal(SDL_Renderer *renderer, SDL_Texture *t,
...
void init_render(SDL_Renderer *renderer) {
  ...
  SDL_SetRenderDrawColor(renderer, (BOARD_BG_COLOR >> 16) & 0xff,
                         (BOARD_BG_COLOR >> 8) & 0xff, BOARD_BG_COLOR & 0xff,
                         0);
  ...
  draw_str(renderer, VERSION_STR, 60 + w + CH_WIDTH, 140 + h - CH_HEIGHT,
           0xffffff);
}
