...
#define SEG_TOT_WIDTH                                                          \
  (SEG_SEP * 18 + SEG_VER_WIDTH * 16 + SEG_DOT_WIDTH * 8 + SEG_HOR_WIDTH * 8)
...
static SDL_Texture *tsegled_ver_off, *tsegled_ver_on, *tsegled_hor_off,
...
static SDL_Rect segs_rect[8] = {
    ...
    {SEG_SEP + SEG_VER_WIDTH + SEG_HOR_WIDTH, SEG_SEP + SEG_HOR_HEIGHT,
     SEG_VER_WIDTH, SEG_VER_HEIGHT},
    {SEG_SEP + SEG_VER_WIDTH + SEG_HOR_WIDTH,
     SEG_SEP + 2 * SEG_HOR_HEIGHT + SEG_VER_HEIGHT, SEG_VER_WIDTH,
     SEG_VER_HEIGHT},
    {SEG_SEP + SEG_VER_WIDTH, SEG_SEP + 2 * SEG_HOR_HEIGHT + 2 * SEG_VER_HEIGHT,
     SEG_HOR_WIDTH, SEG_HOR_HEIGHT},
    {SEG_SEP, SEG_SEP + 2 * SEG_HOR_HEIGHT + SEG_VER_HEIGHT, SEG_VER_WIDTH,
     SEG_VER_HEIGHT},
    {SEG_SEP, SEG_SEP + SEG_HOR_HEIGHT, SEG_VER_WIDTH, SEG_VER_HEIGHT},
    {SEG_SEP + SEG_VER_WIDTH, SEG_SEP + SEG_HOR_HEIGHT + SEG_VER_HEIGHT,
     SEG_HOR_WIDTH, SEG_HOR_HEIGHT},
    {2 * SEG_SEP + 2 * SEG_VER_WIDTH + SEG_HOR_WIDTH,
     SEG_SEP + 3 * SEG_HOR_HEIGHT + 2 * SEG_VER_HEIGHT - SEG_DOT_HEIGHT,
     SEG_DOT_WIDTH, SEG_DOT_HEIGHT}};
...
void SEGS7::update_gui() {
  ...
  for (int i = 0; i < 8; ++i) {
    ...
    SDL_RenderCopy(get_renderer(), get_texture(texture_idx), NULL,
                   get_rect(texture_idx));
  }
  set_redraw();
}
...
static void init_render_local(SDL_Renderer *renderer) {
  // vertical
  tsegled_ver_on =
      new_texture(renderer, SEG_VER_WIDTH, SEG_VER_HEIGHT, 0xff, 0x00, 0x00);
  tsegled_ver_off =
      new_texture(renderer, SEG_VER_WIDTH, SEG_VER_HEIGHT, 0x2b, 0x2b, 0x2b);

  // horizontal
  tsegled_hor_on =
      new_texture(renderer, SEG_HOR_WIDTH, SEG_HOR_HEIGHT, 0xff, 0x00, 0x00);
  tsegled_hor_off =
      new_texture(renderer, SEG_HOR_WIDTH, SEG_HOR_HEIGHT, 0x2b, 0x2b, 0x2b);

  // dot
  tsegled_dot_on =
      new_texture(renderer, SEG_DOT_WIDTH, SEG_DOT_HEIGHT, 0xff, 0x00, 0x00);
  tsegled_dot_off =
      new_texture(renderer, SEG_DOT_WIDTH, SEG_DOT_HEIGHT, 0x2b, 0x2b, 0x2b);
  ...
  const int gap = 14;
  draw_surrounding_line(renderer,
                        Rect(SEG_X, SEG_Y, SEG_TOT_WIDTH, SEG_TOT_HEIGHT), gap);

  // draw indices of each 7-seg display
  SDL_Point p = Point(SEG_X, SEG_Y) + Point(SEG_TOT_WIDTH, SEG_TOT_HEIGHT) +
                Point(0, gap) - Point(SEG_TOT_WIDTH / 8 / 2, 0) -
                Point(CH_WIDTH / 2, CH_HEIGHT / 2);
  char buf[2] = "?";
  for (int i = 0; i < 8; i++) {
    buf[0] = '0' + i;
