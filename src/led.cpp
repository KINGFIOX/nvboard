...
static void init_render_local(SDL_Renderer *renderer) {
  ...
  SDL_Rect r =
      Rect(Point(LED_X, LED_Y) - Point(gap, gap), w + gap * 2, 76 + gap * 2);
...
void init_led(SDL_Renderer *renderer) {
  SDL_Texture *tled_off =
      new_texture(renderer, LED_WIDTH, LED_HEIGHT, 0x7f, 0x7f, 0x7f);
  SDL_Texture *tled_g =
      new_texture(renderer, LED_WIDTH, LED_HEIGHT, 0x00, 0xff, 0x00);
#if 0
  ...
  for (int i = 0; i < 16; ++i) {
    ...
    *rect_ptr = (SDL_Rect){LED_X + (15 - i) * (LED_WIDTH + LED_SEP), LED_Y,
