...
Term::Term(SDL_Renderer *r, int x, int y, int w, int h)
  ...
  get_focus_cursor_texture =
      new_texture(r, CH_WIDTH, CH_HEIGHT, 0xff, 0x00, 0xff);
...
Term::~Term() { SDL_DestroyTexture(cursor_texture); }

void Term::clear_screen() { SDL_RenderFillRect(renderer, &region); }

void Term::set_cursor_visibility(bool v) {
  bool update_cursor_gui = is_cursor_visible ^ v;
  is_cursor_visible = v;
  if (update_cursor_gui)
    draw_cursor();
}
