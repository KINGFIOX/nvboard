...
VGA::VGA(SDL_Renderer *rend, int cnt, int init_val, int ct)
  ...
  *rect_ptr =
      (SDL_Rect){0, WINDOW_HEIGHT / 2, VGA_DEFAULT_WIDTH, VGA_DEFAULT_HEIGHT};
  set_rect(rect_ptr, 0);
  SDL_UpdateTexture(vga_texture, NULL, pixels,
                    vga_screen_width * sizeof(uint32_t));
  ...
  if (is_r_len8)
    p_r = (uint8_t *)pin_array[VGA_R0].ptr;
  if (is_g_len8)
    p_g = (uint8_t *)pin_array[VGA_G0].ptr;
  if (is_b_len8)
    p_b = (uint8_t *)pin_array[VGA_B0].ptr;
...
void VGA::update_state() {
  ...
  uint32_t color = 0;
  if (likely(is_all_len8))
    color = ((*p_r) << 16) | ((*p_g) << 8) | (*p_b);
  else
    color = get_pixel_color_slowpath();
...
void vga_update() { vga->update_state(); }
