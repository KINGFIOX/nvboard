...
#define SCANCODE_LIST(f)                                                       \
  f(_A) f(_B) f(_C) f(_D) f(_E) f(_F) f(_G) f(_H) f(_I) f(_J) f(_K) f(_L) f(   \
      _M) f(_N) f(_O) f(_P) f(_Q) f(_R) f(_S) f(_T) f(_U) f(_V) f(_W) f(_X)    \
      f(_Y) f(_Z) f(_0) f(_1) f(_2) f(_3) f(_4) f(_5) f(_6) f(_7) f(_8) f(     \
          _9) f(_GRAVE) f(_MINUS) f(_EQUALS) f(_BACKSLASH) f(_BACKSPACE)       \
          f(_SPACE) f(_TAB) f(_CAPSLOCK) f(_LSHIFT) f(_LCTRL) f(_LGUI) f(      \
              _LALT) f(_RSHIFT) f(_RCTRL) f(_RGUI) f(_RALT) f(_APPLICATION)    \
              f(_RETURN) f(_ESCAPE) f(_F1) f(_F2) f(_F3) f(_F4) f(_F5) f(_F6)  \
                  f(_F7) f(_F8) f(_F9) f(_F10) f(_F11) f(_F12) f(_PRINTSCREEN) \
                      f(_SCROLLLOCK) f(_LEFTBRACKET) f(_INSERT) f(_HOME)       \
                          f(_PAGEUP) f(_DELETE) f(_END) f(_PAGEDOWN) f(_UP) f( \
                              _LEFT) f(_DOWN) f(_RIGHT) f(_NUMLOCKCLEAR)       \
                              f(_KP_DIVIDE) f(_KP_MULTIPLY)                    \
                                  f(_KP_MINUS) f(_KP_PLUS) f(_KP_ENTER) f(     \
                                      _KP_PERIOD) f(_KP_0) f(_KP_1) f(_KP_2)   \
