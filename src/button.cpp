...
static const SDL_Rect btn_rects[6] = {
    ...
    {BTNC_X + BTNC_WIDTH + BTNC_SEP / 2, BTNC_Y - 2 * (BTNC_HEIGHT + BTNC_SEP),
     BTNC_WIDTH, BTNC_HEIGHT} // RST, but not draw
};
...
static void init_render_local(SDL_Renderer *renderer) {
  ...
  draw_surrounding_line(renderer, Rect(btn_rects[3].x, btn_rects[1].y, w, w),
                        gap);
  ...
  char buf[2] = "?";
  for (int i = 0; i < 5; i++) {
    SDL_Point p = Point(btn_rects[i].x, btn_rects[i].y) +
                  Point(BTNC_WIDTH + 2, BTNC_HEIGHT / 2) -
                  Point(0, CH_HEIGHT / 2);
    buf[0] = str[i];
  ...
  SDL_Point p = Point(btn_rects[3].x, btn_rects[1].y) - Point(0, gap) -
                Point(0, CH_HEIGHT / 2) + Point(w / 2, 0) -
                Point(CH_WIDTH * strlen(str) / 2, 0);
