  ...
  if (left_clk == 0) {
    ...
    if (ps2_clk) {
      ...
      uint8_t ps2_dat = (data_idx == PS2_PARTIAL) ? !UINT8_XOR(all_keys.front())
                        : (data_idx == PS2_STOP)  ? 1
                        : ((data_idx >= PS2_DATA_0) && (data_idx <= PS2_DATA_7))
                            ? (cur_key & 1)
                            : 0;
      pin_poke(PS2_DAT, ps2_dat);
      if ((data_idx >= PS2_DATA_0) && (data_idx <= PS2_DATA_7))
        cur_key >>= 1;
      data_idx++;
    } else if (data_idx == 11) {
      data_idx = 0;
...
static SDL_Surface *new_key_shape(int w, int h) {
  ...
  SDL_Rect r;
  r = (SDL_Rect){.x = 0, .y = 0, .w = 1, .h = h};
  SDL_FillRect(s, &r, black);
  r = (SDL_Rect){.x = 0, .y = 0, .w = w, .h = 1};
  SDL_FillRect(s, &r, black);
  r = (SDL_Rect){.x = w - 1, .y = 0, .w = 1, .h = h};
  SDL_FillRect(s, &r, black);
  r = (SDL_Rect){.x = 0, .y = h - 1, .w = w, .h = 1};
  SDL_FillRect(s, &r, black);
  return s;
}
...
static void init_render_local(SDL_Renderer *renderer) {
  ...
  const int y_top_left =
      y_below_uart + (WINDOW_HEIGHT - y_below_uart - h_keyboard) / 2;
  const int key_gap_before_extend_keys = key_unit_width / 3;

  SDL_Surface *s_1p0 = new_key_shape(key_unit_width, key_unit_width);
  SDL_Surface *s_1p5 = new_key_shape(
      key_unit_width + key_gap + key_unit_width / 2, key_unit_width);
  SDL_Surface *s_2p0 =
      new_key_shape(key_unit_width * 2 + key_gap, key_unit_width);
  SDL_Surface *s_2p25 = new_key_shape(
      key_unit_width * 2 + key_gap * 2 + key_unit_width / 4, key_unit_width);
  SDL_Surface *s_6p0 =
      new_key_shape(key_unit_width * 6 + key_gap * 5, key_unit_width);

  int x = x_top_left, y = y_top_left;

#define _entry(k, desc1, desc2, width)                                         \
  do {                                                                         \
...
#define nextline()                                                             \
  ...
  draw_str(renderer, str, p[2].x - strlen(str) * CH_WIDTH,
           p[2].y - CH_HEIGHT / 2, 0xffffff);
}
...
void kb_update() { kb->update_state(); }
